...
interface ToolUsageMetric {
  toolName: string;
  scenario: string;
  expectedUsage: number; // How many times we expect this tool to be used
  actualUsage: number;   // How many times it was actually used
  correctUsage: number;  // How many times it was used correctly
  missedOpportunities: string[]; // Scenarios where tool should have been used but wasn't
}
...
interface ScenarioTest {
  ...
  criticalTools: string[]; // Tools that MUST be used for scenario success
  setup?: () => Promise<void>;
...
class LLMToolHarness {
  ...
  private async runScenario(scenario: ScenarioTest): Promise<void> {
  ...
  recordToolUsage(toolName: string, scenario: string, wasCorrect: boolean = true): void {
  ...
  private generateToolUsageReport(): void {
